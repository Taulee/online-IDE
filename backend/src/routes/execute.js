...
router.post('/guidance', requirePermission('canRunCode'), async (req, res) => {
  try {
    ...
    if (!executionResult || typeof executionResult !== 'object') {
      return res.status(400).json({ error: 'executionResult 字段不能为空' });
    }

    if (executionResult.success) {
      return res.status(400).json({ error: '只有代码运行失败时才能生成 AI 教学指导' });
    }
    ...
    if (!aiGuidance) {
      return res.status(400).json({ error: '系统尚未配置 AI API，请先让老师在 AI 设置中配置' });
    }
    ...
    return res.status(500).json({ error: error.message || 'AI 教学指导生成失败' });
  }
});

...
- AI API 配置由拥有 `canManageAiSettings` 权限的账号在首页右侧“AI 设置”弹窗中配置，保存后对整个系统生效
...
- `canManageUsers`：用户管理
- `canManageAiSettings`：配置系统级 AI
- `canReviewSubmissions`：查看学生提交

默认权限：

- `teacher`：运行、保存、提交审阅、用户管理、AI 配置
- `student`：运行、保存、提交（不可管理用户/AI 配置/审阅）
...
- `key`（唯一配置名）
- `value`（配置内容，例如明文 AI API 密钥、模型和服务地址）
- `timestamps`
...
- `GET/PUT/DELETE /api/ai-settings`：系统级 AI API 地址、模型和密钥配置（需 AI 配置权限）

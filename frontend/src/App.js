...
function App() {
    ...
    const showOnlyInterface = useCallback((target) => {
        setShowFileManager(target === "fileManager");
        setShowUserManagement(target === "userManagement");
        setShowAiSettings(target === "aiSettings");
        setShowSubmissionPanel(target === "submissionPanel");
        setShowSubmitModal(target === "submitModal");
        setShowAiGuidanceModal(target === "aiGuidanceModal");
    }, []);
    ...
    const handleRun = useCallback(async () => {
        ...
        try {
            ...
            setOutput(
                `[Error] ${getErrorMessage(error, "代码执行请求失败，请稍后重试")}`,
            );
            setLastFailedExecution(null);
        } finally {
            setIsRunning(false);
    ...
    const handleRequestAiGuidance = useCallback(async () => {
        ...
        showOnlyInterface("aiGuidanceModal");
    ...
    }, [lastFailedExecution, logout, showOnlyInterface]);
    ...
    const handleSave = useCallback(async () => {
        ...
        const inputName = prompt(
            "请输入文件名：",
            currentFile?.name || `main.${getExtension(language)}`,
        );
        ...
        try {
            ...
            setOutput(
                `[Error] 保存失败：${getErrorMessage(error, "请稍后重试")}`,
            );
        }
    ...
    const handleFileSelect = useCallback(
        async (file) => {
            try {
                ...
                setOutput(
                    `[Error] 读取文件失败：${getErrorMessage(error, "请稍后重试")}`,
                );
            }
        },
        [logout],
    ...
    const canManageAiSettings = Boolean(user.permissions?.canManageAiSettings);
    const canReviewSubmissions = Boolean(
        user.permissions?.canReviewSubmissions,
    );
    const canSaveFiles = Boolean(user.permissions?.canSaveFiles);
    const canSubmitCode = Boolean(
        user.permissions?.canSubmitCode && user.role === "student",
    );
            ...
            <header className="header">
                <div className="header-left">
                    ...
                    <span className="file-name">
                        {currentFile ? currentFile.name : "未命名"}
                    </span>
                </div>
                ...
                <div className="header-right">
                    {canManageUsers && (
                        <button
                            ...
                            onClick={() => {
                                showOnlyInterface("userManagement");
                            }}
                        >
                            用户管理
                        </button>
                    ...
                    {canReviewSubmissions && (
                        <button
                            ...
                            onClick={() => {
                                ...
                                showOnlyInterface("submissionPanel");
                            }}
                        >
                            学生提交
                        </button>
                    ...
                    {canSaveFiles && (
                        <button
                            ...
                            onClick={() => {
                                showOnlyInterface(
                                    showFileManager ? null : "fileManager",
                                );
                            }}
                        >
                            文件
                    ...
                    {canSubmitCode && (
                        <button
                            className="btn btn-secondary"
                            onClick={() => showOnlyInterface("submitModal")}
                        >
                            提交给老师
                        </button>
                    )}
                    ...
                    <button
                        ...
                        onClick={handleRun}
                        disabled={isRunning || !user.permissions?.canRunCode}
                    >
                        {isRunning ? "运行中..." : "▶ 运行"}
                    </button>
